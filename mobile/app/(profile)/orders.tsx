...
const OrdersScreen = () => {
  ...
  const handleSubmitRating = async() => {
    ...
    try{
      ...
      Alert.alert("Error", "Failed to rate products");
      console.error(error);
    }
  }
  ...
  return (
    <SafeScreen>
      ...
      {isLoading ? (
        <LoadingUI screen="Orders" />
      ) : isError ? (
        <ErrorUI screen="Orders" />
                  ...
                  <View className="flex-row mb-4">
                    ...
                    <View className="flex-1 ml-4">
                      ...
                      <View className="self-start px-3 py-1.5 rounded-full" style={{backgroundColor: getStatusColor(order.status) + "20"}}>
                        <Text className="text-xs font-bold" style={{color: getStatusColor(order.status)}}>{capitalizeFirstLetter(order.status)}</Text>
                      </View>
                    </View>
                  </View>
